...
- Source-level public parser and graph entry points via `phosphene/parser` and `phosphene/graph` exports, plus a declaration-only package build for future standalone extraction
...
| `pnpm build:packages` | Emit parser/graph declaration artifacts under `dist-types/` |
...
- **v0.5+** — Source-level parser/graph package surface and declaration build ✅; standalone npm package publishing remains
...
- Source-Level Public Entry Points fuer Parser und Graph via `phosphene/parser` und `phosphene/graph` plus Declaration-only Package-Build als Vorbereitung fuer spaetere Standalone-Packages
...
| `pnpm build:packages` | Parser-/Graph-Declaration-Artefakte unter `dist-types/` erzeugen |
...
- **v0.5+** — Source-Level Parser/Graph Package Surface und Declaration-Build ✅; Standalone npm Publishing offen
