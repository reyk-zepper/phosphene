...
- Source-level public parser and graph entry points via `phosphene/parser` and `phosphene/graph` exports, plus importable ESM and declaration package builds for future standalone extraction
...
- **v0.5+** — Source-level parser/graph package surface plus ESM/declaration builds ✅; standalone npm package publishing remains
...
- Source-Level Public Entry Points fuer Parser und Graph via `phosphene/parser` und `phosphene/graph` plus importierbare ESM- und Declaration-Package-Builds als Vorbereitung fuer spaetere Standalone-Packages
...
- **v0.5+** — Source-Level Parser/Graph Package Surface plus ESM-/Declaration-Builds ✅; Standalone npm Publishing offen
